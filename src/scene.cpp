...
int Scene::loadGeom(string objectid) {
    ...
    if (id != geoms.size()) {
        ...
        while (!line.empty() && fp_in.good()) {
			...
			} else if (strcmp(tokens[0].c_str(), "T_MOTION") == 0) {
				newGeom.t_motion = glm::vec3(atof(tokens[1].c_str()), atof(tokens[2].c_str()), atof(tokens[3].c_str()));
			} else if (strcmp(tokens[0].c_str(), "R_MOTION") == 0) {
				newGeom.r_motion = glm::vec3(atof(tokens[1].c_str()), atof(tokens[2].c_str()), atof(tokens[3].c_str()));
			}
            utilityCore::safeGetline(fp_in, line);
        }
